...
class SpiderTwitterAccountPost(tool.abc.SingleSpider):
    ...
    def running(self, user_name: str, since_date, until_date):
        ...
        # 判断是否该账号在指定时间范围内没有发文
        label_test = self.driver.find_element_by_css_selector("main > div > div > div > div:nth-child(1) >"
        ...
        for _ in range(1000):
            ...
            for label_tweet in label_outer.find_elements_by_xpath('//*[@id="outer"]/div'):  # 定位到推文标签
                ...
                try:
                    label = label_tweet.find_element_by_css_selector(
                        "article > div > div > div > div:nth-child(2) > div:nth-child(2) > div:n"
                        "th-child(1) > div > div > div:nth-child(1) > a")
                except:
                    break
                if label:
            ...
            if last_label_tweet is not None:
                ...
                self.console(user_name, "执行一次向下翻页...")
                time.sleep(2)
            else:
                break
...
if __name__ == "__main__":
    ...
    num = len(user_names)
    since_date = dt.date(2020, 11, 1)
    until_date = dt.date(2020, 11, 15)
    #待查询动态时间间隔
